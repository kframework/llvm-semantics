; ModuleID = '/home/grosu/celliso2/c-semantics/tests/gcc-torture/20000412-6.c'
...
entry:
  ...
  %tmp1 = load i16** %buffer.addr, align 8
  store i16* %tmp1, i16** %tmp, align 8
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %entry
  %tmp2 = load i16** %tmp, align 8
  %tmp3 = load i16** %bufend.addr, align 8
  %cmp = icmp ult i16* %tmp2, %tmp3
  br i1 %cmp, label %for.body, label %for.end

for.body:                                         ; preds = %for.cond
  %tmp4 = load i16** %tmp, align 8
  %tmp5 = load i16* %tmp4
  %conv = zext i16 %tmp5 to i32
  %tmp6 = load i16* %value.addr, align 2
  %conv7 = zext i16 %tmp6 to i32
  %sub = sub nsw i32 %conv7, %conv
  %conv8 = trunc i32 %sub to i16
  store i16 %conv8, i16* %value.addr, align 2
  br label %for.inc

for.inc:                                          ; preds = %for.body
  %tmp9 = load i16** %tmp, align 8
  %incdec.ptr = getelementptr inbounds i16* %tmp9, i32 1
  store i16* %incdec.ptr, i16** %tmp, align 8
  br label %for.cond

for.end:                                          ; preds = %for.cond
  %tmp10 = load i16* %value.addr, align 2
