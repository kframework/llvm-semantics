; ModuleID = '/home/david/src/c-semantics/tests/csmith/smith000071.c'
target datalayout = "e-p:64:64:64-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:64:64-f32:32:32-f64:64:64-v64:64:64-v128:128:128-a0:0:64-s0:64:64-f80:128:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

@g_8 = constant [2 x [5 x [3 x [1 x i32]]]] [[5 x [3 x [1 x i32]]] [[3 x [1 x i32]] [[1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061]], [3 x [1 x i32]] [[1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061]], [3 x [1 x i32]] [[1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061]], [3 x [1 x i32]] [[1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061]], [3 x [1 x i32]] [[1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061]]], [5 x [3 x [1 x i32]]] [[3 x [1 x i32]] [[1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061]], [3 x [1 x i32]] [[1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061]], [3 x [1 x i32]] [[1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061]], [3 x [1 x i32]] [[1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061]], [3 x [1 x i32]] [[1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061], [1 x i32] [i32 -508159061]]]], align 16
@g_15 = constant i32 1388262280, align 4
@g_14 = global i32* @g_15, align 8
@g_16 = global i32** @g_14, align 8
@g_17 = global i32** null, align 8
@g_19 = global i32 -1, align 4
@g_18 = global i32* @g_19, align 8
@.str = private unnamed_addr constant [31 x i8] c"checksum g_8[0][0][0][0] = %d\0A\00", align 1
@.str1 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][0][1][0] = %d\0A\00", align 1
@.str2 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][0][2][0] = %d\0A\00", align 1
@.str3 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][1][0][0] = %d\0A\00", align 1
@.str4 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][1][1][0] = %d\0A\00", align 1
@.str5 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][1][2][0] = %d\0A\00", align 1
@.str6 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][2][0][0] = %d\0A\00", align 1
@.str7 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][2][1][0] = %d\0A\00", align 1
@.str8 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][2][2][0] = %d\0A\00", align 1
@.str9 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][3][0][0] = %d\0A\00", align 1
@.str10 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][3][1][0] = %d\0A\00", align 1
@.str11 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][3][2][0] = %d\0A\00", align 1
@.str12 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][4][0][0] = %d\0A\00", align 1
@.str13 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][4][1][0] = %d\0A\00", align 1
@.str14 = private unnamed_addr constant [31 x i8] c"checksum g_8[0][4][2][0] = %d\0A\00", align 1
@.str15 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][0][0][0] = %d\0A\00", align 1
@.str16 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][0][1][0] = %d\0A\00", align 1
@.str17 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][0][2][0] = %d\0A\00", align 1
@.str18 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][1][0][0] = %d\0A\00", align 1
@.str19 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][1][1][0] = %d\0A\00", align 1
@.str20 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][1][2][0] = %d\0A\00", align 1
@.str21 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][2][0][0] = %d\0A\00", align 1
@.str22 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][2][1][0] = %d\0A\00", align 1
@.str23 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][2][2][0] = %d\0A\00", align 1
@.str24 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][3][0][0] = %d\0A\00", align 1
@.str25 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][3][1][0] = %d\0A\00", align 1
@.str26 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][3][2][0] = %d\0A\00", align 1
@.str27 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][4][0][0] = %d\0A\00", align 1
@.str28 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][4][1][0] = %d\0A\00", align 1
@.str29 = private unnamed_addr constant [31 x i8] c"checksum g_8[1][4][2][0] = %d\0A\00", align 1
@.str30 = private unnamed_addr constant [20 x i8] c"checksum g_15 = %d\0A\00", align 1
@.str31 = private unnamed_addr constant [20 x i8] c"checksum g_19 = %d\0A\00", align 1
@__undefined = common global i64 0, align 8

define i32 @func_1() nounwind uwtable {
  %l_13 = alloca i32*, align 8
  %l_20 = alloca i32*, align 8
  store i32* null, i32** %l_13, align 8
  store i32* @g_19, i32** %l_20, align 8
  %1 = load volatile i32** @g_14, align 8
  %2 = load volatile i32*** @g_16, align 8
  store volatile i32* %1, i32** %2
  %3 = load volatile i32* @g_15, align 4
  %4 = trunc i32 %3 to i16
  %5 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 3, i64 1, i64 0), align 4
  %6 = load i32** %l_13, align 8
  %7 = call i32* @func_9(i16 signext %4, i32 %5, i32* %6)
  store i32* %7, i32** @g_18, align 8
  %8 = load i32** %l_20, align 8
  %9 = load i32* %8
  %10 = sext i32 %9 to i64
  %11 = or i64 %10, 3770597429
  %12 = trunc i64 %11 to i32
  store i32 %12, i32* %8
  %13 = load i32* @g_19, align 4
  ret i32 %13
}

define i32* @func_9(i16 signext %p_10, i32 %p_11, i32* %p_12) nounwind uwtable {
  %1 = alloca i16, align 2
  %2 = alloca i32, align 4
  %3 = alloca i32*, align 8
  store i16 %p_10, i16* %1, align 2
  store i32 %p_11, i32* %2, align 4
  store i32* %p_12, i32** %3, align 8
  %4 = load i32** %3, align 8
  ret i32* %4
}

define i32* @func_2(i32* %p_3) nounwind uwtable {
  %1 = alloca i32*, align 8
  %l_6 = alloca [4 x i16], align 2
  %l_7 = alloca i32*, align 8
  %i = alloca i32, align 4
  store i32* %p_3, i32** %1, align 8
  store i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 2, i64 0, i64 0), i32** %l_7, align 8
  store i32 0, i32* %i, align 4
  br label %2

; <label>:2                                       ; preds = %9, %0
  %3 = load i32* %i, align 4
  %4 = icmp slt i32 %3, 4
  br i1 %4, label %5, label %12

; <label>:5                                       ; preds = %2
  %6 = load i32* %i, align 4
  %7 = sext i32 %6 to i64
  %8 = getelementptr inbounds [4 x i16]* %l_6, i32 0, i64 %7
  store i16 29980, i16* %8, align 2
  br label %9

; <label>:9                                       ; preds = %5
  %10 = load i32* %i, align 4
  %11 = add nsw i32 %10, 1
  store i32 %11, i32* %i, align 4
  br label %2

; <label>:12                                      ; preds = %2
  %13 = getelementptr inbounds [4 x i16]* %l_6, i32 0, i64 3
  %14 = load i16* %13, align 2
  %15 = zext i16 %14 to i64
  %16 = icmp sge i64 0, %15
  %17 = zext i1 %16 to i32
  %18 = trunc i32 %17 to i8
  %19 = call signext i8 @safe_rshift_func_int8_t_s_s(i8 signext %18, i32 2119501211)
  %20 = sext i8 %19 to i32
  %21 = load i32** %1, align 8
  %22 = load i32* %21
  %23 = and i32 %22, %20
  store i32 %23, i32* %21
  %24 = load i32** %l_7, align 8
  ret i32* %24
}

define internal signext i8 @safe_rshift_func_int8_t_s_s(i8 signext %left, i32 %right) nounwind uwtable {
  %1 = alloca i8, align 1
  %2 = alloca i32, align 4
  store i8 %left, i8* %1, align 1
  store i32 %right, i32* %2, align 4
  %3 = load i8* %1, align 1
  %4 = sext i8 %3 to i32
  %5 = icmp slt i32 %4, 0
  br i1 %5, label %12, label %6

; <label>:6                                       ; preds = %0
  %7 = load i32* %2, align 4
  %8 = icmp slt i32 %7, 0
  br i1 %8, label %12, label %9

; <label>:9                                       ; preds = %6
  %10 = load i32* %2, align 4
  %11 = icmp sge i32 %10, 32
  br i1 %11, label %12, label %15

; <label>:12                                      ; preds = %9, %6, %0
  %13 = load i8* %1, align 1
  %14 = sext i8 %13 to i32
  br label %20

; <label>:15                                      ; preds = %9
  %16 = load i8* %1, align 1
  %17 = sext i8 %16 to i32
  %18 = load i32* %2, align 4
  %19 = ashr i32 %17, %18
  br label %20

; <label>:20                                      ; preds = %15, %12
  %21 = phi i32 [ %14, %12 ], [ %19, %15 ]
  %22 = trunc i32 %21 to i8
  ret i8 %22
}

define i32 @main(i32 %argc, i8** %argv) nounwind uwtable {
  %1 = alloca i32, align 4
  %2 = alloca i32, align 4
  %3 = alloca i8**, align 8
  %i = alloca i32, align 4
  %j = alloca i32, align 4
  %k = alloca i32, align 4
  %l = alloca i32, align 4
  store i32 0, i32* %1
  store i32 %argc, i32* %2, align 4
  store i8** %argv, i8*** %3, align 8
  %4 = call i32 @func_1()
  %5 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 0, i64 0, i64 0), align 4
  %6 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str, i32 0, i32 0), i32 %5)
  %7 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 0, i64 1, i64 0), align 4
  %8 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str1, i32 0, i32 0), i32 %7)
  %9 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 0, i64 2, i64 0), align 4
  %10 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str2, i32 0, i32 0), i32 %9)
  %11 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 1, i64 0, i64 0), align 4
  %12 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str3, i32 0, i32 0), i32 %11)
  %13 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 1, i64 1, i64 0), align 4
  %14 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str4, i32 0, i32 0), i32 %13)
  %15 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 1, i64 2, i64 0), align 4
  %16 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str5, i32 0, i32 0), i32 %15)
  %17 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 2, i64 0, i64 0), align 4
  %18 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str6, i32 0, i32 0), i32 %17)
  %19 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 2, i64 1, i64 0), align 4
  %20 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str7, i32 0, i32 0), i32 %19)
  %21 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 2, i64 2, i64 0), align 4
  %22 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str8, i32 0, i32 0), i32 %21)
  %23 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 3, i64 0, i64 0), align 4
  %24 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str9, i32 0, i32 0), i32 %23)
  %25 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 3, i64 1, i64 0), align 4
  %26 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str10, i32 0, i32 0), i32 %25)
  %27 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 3, i64 2, i64 0), align 4
  %28 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str11, i32 0, i32 0), i32 %27)
  %29 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 4, i64 0, i64 0), align 4
  %30 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str12, i32 0, i32 0), i32 %29)
  %31 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 4, i64 1, i64 0), align 4
  %32 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str13, i32 0, i32 0), i32 %31)
  %33 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 0, i64 4, i64 2, i64 0), align 4
  %34 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str14, i32 0, i32 0), i32 %33)
  %35 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 0, i64 0, i64 0), align 4
  %36 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str15, i32 0, i32 0), i32 %35)
  %37 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 0, i64 1, i64 0), align 4
  %38 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str16, i32 0, i32 0), i32 %37)
  %39 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 0, i64 2, i64 0), align 4
  %40 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str17, i32 0, i32 0), i32 %39)
  %41 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 1, i64 0, i64 0), align 4
  %42 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str18, i32 0, i32 0), i32 %41)
  %43 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 1, i64 1, i64 0), align 4
  %44 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str19, i32 0, i32 0), i32 %43)
  %45 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 1, i64 2, i64 0), align 4
  %46 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str20, i32 0, i32 0), i32 %45)
  %47 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 2, i64 0, i64 0), align 4
  %48 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str21, i32 0, i32 0), i32 %47)
  %49 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 2, i64 1, i64 0), align 4
  %50 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str22, i32 0, i32 0), i32 %49)
  %51 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 2, i64 2, i64 0), align 4
  %52 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str23, i32 0, i32 0), i32 %51)
  %53 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 3, i64 0, i64 0), align 4
  %54 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str24, i32 0, i32 0), i32 %53)
  %55 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 3, i64 1, i64 0), align 4
  %56 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str25, i32 0, i32 0), i32 %55)
  %57 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 3, i64 2, i64 0), align 4
  %58 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str26, i32 0, i32 0), i32 %57)
  %59 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 4, i64 0, i64 0), align 4
  %60 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str27, i32 0, i32 0), i32 %59)
  %61 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 4, i64 1, i64 0), align 4
  %62 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str28, i32 0, i32 0), i32 %61)
  %63 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 4, i64 2, i64 0), align 4
  %64 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str29, i32 0, i32 0), i32 %63)
  %65 = load i32* getelementptr inbounds ([2 x [5 x [3 x [1 x i32]]]]* @g_8, i32 0, i64 1, i64 2, i64 0, i64 0), align 4
  %66 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([31 x i8]* @.str21, i32 0, i32 0), i32 %65)
  %67 = load volatile i32* @g_15, align 4
  %68 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([20 x i8]* @.str30, i32 0, i32 0), i32 %67)
  %69 = load i32* @g_19, align 4
  %70 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([20 x i8]* @.str31, i32 0, i32 0), i32 %69)
  %71 = load i32* %1
  ret i32 %71
}

declare i32 @printf(i8*, ...)
