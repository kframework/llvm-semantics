; ModuleID = '/home/david/src/c-semantics/tests/unitTests/floats-ull.c'
target datalayout = "e-p:64:64:64-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:64:64-f32:32:32-f64:64:64-v64:64:64-v128:128:128-a0:0:64-s0:64:64-f80:128:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

@.str = private unnamed_addr constant [15 x i8] c"Messed up 1.99\00", align 1
@.str1 = private unnamed_addr constant [23 x i8] c"VOLATILE: Messed up &&\00", align 1
@.str2 = private unnamed_addr constant [13 x i8] c"Messed up ~~\00", align 1
@.str3 = private unnamed_addr constant [14 x i8] c"Messed up 0.0\00", align 1

define i64 @f2ull(float %f) nounwind uwtable {
entry:
  %f.addr = alloca float, align 4
  store float %f, float* %f.addr, align 4
  %0 = load float* %f.addr, align 4
  %conv = fptoui float %0 to i64
  ret i64 %conv
}

define i64 @d2ull(double %d) nounwind uwtable {
entry:
  %d.addr = alloca double, align 8
  store double %d, double* %d.addr, align 8
  %0 = load double* %d.addr, align 8
  %conv = fptoui double %0 to i64
  ret i64 %conv
}

define i32 @main() nounwind uwtable {
entry:
  %retval = alloca i32, align 4
  %innerInt = alloca i64, align 8
  %innerFloat = alloca float, align 4
  %lhs_ull = alloca i64, align 8
  %choice1 = alloca i64, align 8
  %choice2 = alloca i64, align 8
  store i32 0, i32* %retval
  %call = call i64 @f2ull(float 0x3FFFD70A40000000)
  %cmp = icmp ne i64 %call, 1
  br i1 %cmp, label %if.then, label %if.end

if.then:                                          ; preds = %entry
  %call1 = call i32 @puts(i8* getelementptr inbounds ([15 x i8]* @.str, i32 0, i32 0))
  br label %if.end

if.end:                                           ; preds = %if.then, %entry
  store i64 9223372036854775807, i64* %innerInt, align 8
  %0 = load i64* %innerInt, align 8
  %conv = uitofp i64 %0 to float
  store float %conv, float* %innerFloat, align 4
  %1 = load float* %innerFloat, align 4
  %call2 = call i64 @f2ull(float %1)
  store i64 %call2, i64* %lhs_ull, align 8
  store i64 9223372036854775807, i64* %choice1, align 8
  store i64 -9223372036854775808, i64* %choice2, align 8
  %2 = load i64* %lhs_ull, align 8
  %3 = load i64* %choice1, align 8
  %cmp3 = icmp ne i64 %2, %3
  br i1 %cmp3, label %land.lhs.true, label %if.end9

land.lhs.true:                                    ; preds = %if.end
  %4 = load i64* %lhs_ull, align 8
  %5 = load i64* %choice2, align 8
  %cmp5 = icmp ne i64 %4, %5
  br i1 %cmp5, label %if.then7, label %if.end9

if.then7:                                         ; preds = %land.lhs.true
  %call8 = call i32 @puts(i8* getelementptr inbounds ([23 x i8]* @.str1, i32 0, i32 0))
  br label %if.end9

if.end9:                                          ; preds = %if.then7, %land.lhs.true, %if.end
  %call10 = call i64 @f2ull(float 0x43E0000000000000)
  %cmp11 = icmp ne i64 %call10, -9223372036854775808
  br i1 %cmp11, label %if.then13, label %if.end15

if.then13:                                        ; preds = %if.end9
  %call14 = call i32 @puts(i8* getelementptr inbounds ([13 x i8]* @.str2, i32 0, i32 0))
  br label %if.end15

if.end15:                                         ; preds = %if.then13, %if.end9
  %call16 = call i64 @d2ull(double 0.000000e+00)
  %cmp17 = icmp ne i64 %call16, 0
  br i1 %cmp17, label %if.then19, label %if.end21

if.then19:                                        ; preds = %if.end15
  %call20 = call i32 @puts(i8* getelementptr inbounds ([14 x i8]* @.str3, i32 0, i32 0))
  br label %if.end21

if.end21:                                         ; preds = %if.then19, %if.end15
  ret i32 0
}

declare i32 @puts(i8*)
