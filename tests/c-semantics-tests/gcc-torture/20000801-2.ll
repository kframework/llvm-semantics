...
while.cond:                                       ; preds = %if.end, %entry
  %0 = load %struct.foo** %node.addr, align 8
...
if.end:                                           ; preds = %land.lhs.true, %while.body
  %1 = load %struct.foo** %node.addr, align 8
  %next = getelementptr inbounds %struct.foo* %1, i32 0, i32 0
  %2 = load %struct.foo** %next, align 8
  store %struct.foo* %2, %struct.foo** %node.addr, align 8
  br label %while.cond

while.end:                                        ; preds = %if.then, %while.cond
  %3 = load %struct.foo** %node.addr, align 8
  ret %struct.foo* %3
}
