; ModuleID = '/home/grosu/celliso2/c-semantics/tests/gcc-torture/990829-1.c'
...
entry:
  ...
  %tmp = load double* %ri.addr, align 8
  %tmp1 = load double* %le.addr, align 8
  %sub = fsub double %tmp, %tmp1
  %tmp2 = load double* %ri.addr, align 8
  %tmp3 = load double* %le.addr, align 8
  %add = fadd double %tmp3, 1.000000e+00
  %mul = fmul double %tmp2, %add
  %div = fdiv double %sub, %mul
  store double %div, double* %val, align 8
  %tmp4 = load double* %val, align 8
  ret double %tmp4
}
...
entry:
  ...
  %tmp = load double* %retval1, align 8
  %cmp = fcmp olt double %tmp, 2.400000e-01
  br i1 %cmp, label %if.then, label %lor.lhs.false

lor.lhs.false:                                    ; preds = %entry
  %tmp2 = load double* %retval1, align 8
  %cmp3 = fcmp ogt double %tmp2, 2.600000e-01
