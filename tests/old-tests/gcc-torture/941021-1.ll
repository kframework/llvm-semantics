; ModuleID = '/home/grosu/celliso2/c-semantics/tests/gcc-torture/941021-1.c'
...
entry:
  ...
  %tmp = load double** %pdbl.addr, align 8
  %cmp = icmp eq double* %tmp, null
...
if.end:                                           ; preds = %if.then, %entry
  %tmp1 = load double* %value.addr, align 8
  %tmp2 = load double** %pdbl.addr, align 8
  store double %tmp1, double* %tmp2
  ret void
}
...
entry:
  ...
  %tmp = load double* @glob_dbl, align 8
  %cmp = fcmp une double %tmp, 5.510000e+01
  br i1 %cmp, label %if.then, label %if.end
