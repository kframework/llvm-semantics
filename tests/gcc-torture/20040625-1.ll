; ModuleID = '/home/grosu/celliso2/c-semantics/tests/gcc-torture/20040625-1.c'
...
entry:
  ...
  %tmp = load i32* %t.addr, align 4
  %tobool = icmp ne i32 %tmp, 0
  br i1 %tobool, label %if.then, label %if.end

if.then:                                          ; preds = %entry
  %tmp1 = load %struct.s** %s.addr, align 8
  %next = getelementptr inbounds %struct.s* %tmp1, i32 0, i32 0
  %tmp2 = load %struct.s** %next, align 1
  store %struct.s* %tmp2, %struct.s** %s.addr, align 8
  br label %if.end

if.end:                                           ; preds = %if.then, %entry
  %tmp3 = load %struct.s** %s.addr, align 8
  ret %struct.s* %tmp3
}
